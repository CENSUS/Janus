...
const {
    addCAMajorityReached,
    removeCAMajorityReached,
} = require("../../helper");
...
class ElectionManager extends StateConnector {
    ...
    async updateFromElection(ctx, election) {
        switch (election.electionInfo.electionType) {
            case "add_ca":
                await addCAMajorityReached(ctx, election);
                break;
            case "remove_ca":
                await removeCAMajorityReached(ctx, election);
                break;
            default:
                break;
        }
    ...
    async startElection(cid, electionInstance) {
        ...
        return [communicator.response, communicator.error];
    }
}
